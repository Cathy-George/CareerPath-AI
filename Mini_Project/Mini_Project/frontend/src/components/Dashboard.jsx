    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8 w-full max-w-6xl mx-auto pb-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-slate-800/50 p-8 rounded-2xl border border-slate-700/50">
                    <h2 className="text-3xl font-bold text-white mb-2">{job_match_analysis.seniority_level} Role Analysis</h2>
                    <p className="text-slate-400 mb-6">{job_match_analysis.reasoning}</p>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-xl">
                            <span className="text-emerald-400 text-sm font-semibold">Strong Skills</span>
                            <div className="mt-2 flex flex-wrap gap-2">
                                {skill_gap_report.strong.map(s => <span key={s} className="text-xs bg-emerald-500/20 text-emerald-300 px-2 py-1 rounded-full">{s}</span>)}
                            </div>
                        </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                        {project_suggestions.map((project, i) => (
                            <div key={i} className="bg-slate-800/50 p-6 rounded-xl border border-slate-700/50 hover:bg-slate-800/80 transition-all">
